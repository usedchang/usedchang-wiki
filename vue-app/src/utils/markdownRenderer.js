    ...
    highlight(str, lang) {
      ...
      if (resolvedLanguage && hljs.getLanguage(resolvedLanguage)) {
        try {
          ...
          return `<pre class="hljs"><div class="code-header"><span class="code-lang">${safeLang}</span><button type="button" class="copy-btn" aria-label="复制代码">复制</button></div><code class="hljs language-${langClass}">${highlighted}</code></pre>`;
        } catch {
          /* fall through */
        }
      }
      return `<pre class="hljs"><div class="code-header"><span class="code-lang">TEXT</span><button type="button" class="copy-btn" aria-label="复制代码">复制</button></div><code>${escapeHtml(str)}</code></pre>`;
    },
  })
...
function copyCodeFromButton(button) {
  ...
  if (code == null || button.classList.contains("copied")) return;
